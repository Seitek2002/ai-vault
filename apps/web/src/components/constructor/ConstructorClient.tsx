...
import type { LetterheadDto } from "@/lib/api/letterheads";
...
// ─── Letterhead create / edit modal ────────────────────────────────────────────

function LetterheadModal({ initial, onClose }: { initial?: LetterheadDto; onClose: () => void }) {
  const qc = useQueryClient();
  const [name, setName] = useState(initial?.name ?? "");
  const [bodyJson, setBodyJson] = useState<unknown>(initial?.bodyJson ?? EMPTY_BODY);
  const isEdit = !!initial;
  ...
  const mutation = useMutation({
    mutationFn: () => {
      const base = { name: name.trim() || "Колонтитул", bodyJson };
      return isEdit ? letterheadsApi.update(initial.id, base) : letterheadsApi.create(base);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["letterheads"] });
      onClose();
    },
  });
  ...
  return (
    <Modal onClose={onClose} className="max-w-3xl flex flex-col max-h-[90vh]">
      <div className="px-6 py-4 border-b border-[var(--color-border)] flex items-center justify-between shrink-0">
        <h2 className="text-base font-semibold text-[var(--color-text-primary)]">
          {isEdit ? "Редактировать колонтитул" : "Новый колонтитул"}
        </h2>
        <button onClick={onClose} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors">
          <X className="w-5 h-5" />
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        <div>
          <label className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider block mb-1.5">
            Название колонтитула
          </label>
          <Input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Например, Шапка на трёх языках"
            autoFocus
          />
        </div>
        ...
        <div>
          ...
          <div className="rounded-xl border border-[var(--color-border)] overflow-hidden" style={{ height: 320 }}>
            <RichEditor
              initialContent={bodyJson}
              onChange={setBodyJson}
              placeholder="Соберите колонтитул: логотип, таблица, текст…"
            />
          </div>
        </div>
      </div>
...
function LetterheadCard({
  ...
  return (
    <Card hoverable className="group flex items-center gap-4 px-5 py-4">
      <div className="mt-0.5 w-1 self-stretch rounded-full shrink-0 bg-[var(--color-accent)]" />
      <p className="flex-1 min-w-0 text-sm font-medium text-[var(--color-text-primary)] truncate">
        {letterhead.name}
      </p>
      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
...
function LetterheadsSection() {
  ...
  const [editing, setEditing] = useState<LetterheadDto | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => letterheadsApi.delete(id),
  ...
  return (
    <div className="mt-8">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h2 className="text-sm font-semibold text-[var(--color-text-primary)]">Колонтитулы</h2>
          <p className="text-xs text-[var(--color-text-muted)] mt-0.5">
            Готовые шапки документов — доступны из тулбара редактора при заполнении любого документа или шаблона
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-12 text-[var(--color-accent)]">
          <Spinner size="lg" />
        </div>
      ) : letterheads.length === 0 ? (
        <EmptyState
          icon={<FileStack className="w-6 h-6" strokeWidth={1.5} />}
          title="Своих колонтитулов пока нет"
          description="Соберите шапку документа один раз, чтобы вставлять её из тулбара редактора"
          action={
            <Button variant="secondary" onClick={() => setShowCreate(true)}>
              <Plus className="w-4 h-4" strokeWidth={2.5} />
              Создать колонтитул
        ...
        <div className="grid gap-2">
          {letterheads.map((lh) => (
            <LetterheadCard
              key={lh.id}
              letterhead={lh}
              onEdit={() => setEditing(lh)}
              onDelete={() => {
                if (confirm(`Удалить колонтитул "${lh.name}"?`)) {
      ...
      {showCreate && <LetterheadModal onClose={() => setShowCreate(false)} />}
      {editing && <LetterheadModal initial={editing} onClose={() => setEditing(null)} />}
    </div>
  );
}
