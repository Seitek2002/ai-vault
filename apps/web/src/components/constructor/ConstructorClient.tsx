  ...
  return (
    <div className="p-6 lg:p-8 h-full flex flex-col">
      <div className="mb-6 flex items-center justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-xl font-semibold text-[var(--color-text-primary)]">Конструктор шаблонов</h1>
          <p className="mt-0.5 text-sm text-[var(--color-text-secondary)]">
            Создавайте свои документы с переменными полями, которые появляются слева при заполнении
          </p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto -mx-6 px-6 lg:-mx-8 lg:px-8">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center border border-dashed border-[var(--color-border)] rounded-xl">
            ...
            <p className="text-sm font-medium text-[var(--color-text-secondary)]">Своих шаблонов пока нет</p>
            <p className="mt-1 text-sm text-[var(--color-text-muted)]">
              Создайте документ с переменными полями
            </p>
            <button
              onClick={() => setShowCreate(true)}
