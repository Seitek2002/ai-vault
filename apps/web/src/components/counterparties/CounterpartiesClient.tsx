...
function CounterpartyModal({ editing, onClose, onSaved }: ModalProps) {
      ...
      <div className="relative w-full max-w-lg bg-[var(--color-bg-surface)] rounded-2xl border border-[var(--color-border)] shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
          <h2 className="text-base font-semibold text-[var(--color-text-primary)]">
            {editing ? 'Редактировать контрагента' : 'Новый контрагент'}
          </h2>
          <button
            onClick={onClose}
...
function DeleteModal({ cp, onClose, onDeleted }: DeleteModalProps) {
  const qc = useQueryClient();
  const mutation = useMutation({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-[var(--color-bg-surface)] rounded-2xl border border-[var(--color-border)] shadow-2xl p-6">
        ...
        <p className="text-sm text-[var(--color-text-secondary)] mb-5">
          <span className="font-medium text-[var(--color-text-primary)]">{cp.name}</span> будет удалён. Это действие нельзя отменить.
        </p>
...
export function CounterpartiesClient() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['counterparties', debouncedSearch],
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)] bg-[var(--color-bg-surface)] shrink-0">
        <div>
          <h1 className="text-base font-semibold text-[var(--color-text-primary)]">Контрагенты</h1>
          <p className="text-xs text-[var(--color-text-muted)] mt-0.5">
            Организации и партнёры
          </p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        ...
        {isError && (
          <div className="flex items-center justify-center h-48">
            <p className="text-sm text-[var(--color-text-muted)]">Не удалось загрузить контрагентов</p>
          </div>
        )}
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-[var(--color-text-primary)]">
                {debouncedSearch ? 'Контрагенты не найдены' : 'Нет контрагентов'}
              </p>
              <p className="text-xs text-[var(--color-text-muted)] mt-1">
                {debouncedSearch ? 'Попробуйте изменить поисковый запрос' : 'Добавьте первого контрагента'}
              </p>
            </div>
            {!debouncedSearch && (
              <button
